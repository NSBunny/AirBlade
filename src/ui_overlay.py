...
def draw_trail(frame: np.ndarray, points: Sequence[Point]) -> None:
    if len(points) < 2:
        return
    # Optimization: Draw glow on a downscaled buffer
    h, w = frame.shape[:2]
    small_h, small_w = h // 4, w // 4
    if small_h <= 0 or small_w <= 0:
        return

    small_glow = np.zeros((small_h, small_w, 3), dtype=np.uint8)
    
    # Scale points for the small buffer
    small_points = [(int(p[0] / 4), int(p[1] / 4)) for p in points]
    ...
    for i in range(1, len(points)):
        ...
        # Draw on small buffer for glow
        cv2.line(small_glow, small_points[i - 1], small_points[i], color, max(1, thickness // 4), cv2.LINE_AA)
        
        # Draw core directly on main frame
        cv2.line(frame, points[i - 1], points[i], (255, 255, 255), max(2, thickness // 3), cv2.LINE_AA)

    # Blur the small buffer
    small_glow = cv2.GaussianBlur(small_glow, (15, 15), 0)
    
    # Upscale and add
    glow = cv2.resize(small_glow, (w, h), interpolation=cv2.INTER_LINEAR)
